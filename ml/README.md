...
## Mendeley 3-plant pilot

`.github/workflows/train-model.yml` downloads **only** the original 9.88 MB CC BY 4.0 dataset from the [versioned Mendeley record](https://data.mendeley.com/datasets/7wbstnfpjy/2), validates nine classes, trains, and uploads model weights as a short-lived GitHub Actions artifact. The 47.6 MB augmented and 329 MB diffusion datasets are deliberately excluded. The exact class folders and training output have not yet been inspected in a successful run. Any failed source validation stops the build. Do not publish this model before testing on independently collected houseplant photos and adding proper attribution.
